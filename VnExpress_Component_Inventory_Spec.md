...
## 0.1 — Color Tokens
*(Trích từ artboard Color — xác nhận lại mã hex chính xác từ file gốc)*
...
**Divider & Background**
- `divider/default` `#E5E5E5` (*be always used!*) · `bg/note` `#FCFAF8` · `bg/dark` `#17171A`
...
⚠️ **NHÃN HEX SAI trong file gốc — phải lấy lại bằng eyedropper từ swatch** *(số dưới chỉ là ước lượng từ ảnh để đối chiếu, KHÔNG dùng làm giá trị cuối):*
...
**Advertisement** — Orange `#FDDC79` (nền tag tài trợ). *(Lưu ý: artboard mới ghi `ADVERTISEMENT ICON #5D6C79` — đây cũng là nhãn xám dán nhầm; màu icon ad thực tế là cam → lấy lại từ swatch.)*
...
- *(Kiểm chứng: 12×66 + 11×28 = **1100** ✓ — khớp Masthead 1100 & Breakpage 1100)*
...
- Radius tokens (CHỐT):
  ...
  - *(PC thumb: mặc định 0px — chốt lại nếu muốn đồng bộ 3px cho cả PC)*
...
- **Dạng bài** (loại bài) ⚠️ *quan trọng — map sang type-icon của Block tin*
...
[Title]🔌
[Lead/Sapo]🔌
[Meta: timestamp🔌 + location🔌 + category🔌 + comment-count🔌]
```
...
| O05 | **Box 300 (sidebar)** ⚠️ *(ngoài phạm vi Landingpage / không có trên Mobile)* | biến thể: `Ảnh` · `Video` · `Longform` · `Bảng giá` · `Tỷ giá/Lãi suất` · `Tác giả nổi bật` · `Đánh giá thành lý` · `Internet banking` · `Lịch thi đấu` · `Chủ đề hot` · `Bài viết trong tay` |
...
| `comment_count` | number | icon + số; = 0 thì ẩn | |
...
**Organisms**
- [ ] Header (PC/Mobile) · [ ] Footer · [ ] Topstory · [ ] List/Stream · [ ] Box 300 *(ngoài LDP)* · [ ] Comment section · [ ] Tin liên quan · [ ] Theo dòng sự kiện · [ ] Xem nhiều · [ ] Native Ad (8 loại) · [ ] **Ad slots (Masthead/Large 1/Breakpage/Background)** · [ ] Vote · [ ] Popups (P01–P10)
...
1. **6 Folder colors sai nhãn hex** (Du lịch, Khoa học, Số hóa, Cười, Giải trí, Pháp luật) + **icon Advertisement** — lấy hex thật bằng eyedropper từ swatch file gốc, sửa luôn nhãn. *(Bảng chi tiết ở mục 0.1.)*
